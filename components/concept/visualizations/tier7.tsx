...
// ── 1. Limit Table: auto-add x-values approaching a ────────────────────────
export function LimitTableViz({ accent }: VizProps) {
  ...
  useEffect(() => {
    const t = setInterval(() => setRows(p => (p >= xVals.length ? 1 : p + 1)), 1500);
  ...
  return (
    <View>
      <Svg width={W} height={H}>
        ...
        <SvgText x={W / 2} y={H - 4} fill={colors.text3} fontSize={11} textAnchor="middle">L = 4</SvgText>
      </Svg>
    </View>
  );
}

// ── 2. Secant → Tangent: auto-cycle dx values ──────────────────────────────
export function SecantTangentViz({ accent }: VizProps) {
  const dxVals = [W * 0.37, W * 0.22, W * 0.10, W * 0.04];
  const [i, setI] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setI(p => (p + 1) % dxVals.length), 1800);
    return () => clearInterval(t);
  }, []);
  ...
  const sx1 = p1px - ext * (dx / safeDx);
  const sy1 = p1py - ext * (dy / safeDx);
  const sx2 = p2px + ext * (dx / safeDx);
  const sy2 = p2py + ext * (dy / safeDx);
  ...
  return (
    <View>
      <Svg width={W} height={H}>
        ...
        <Line x1={sx1} y1={sy1} x2={sx2} y2={sy2} stroke={accent} strokeWidth={2} />
        <Circle cx={p1px} cy={p1py} r={5} fill={accent} />
        <Circle cx={p2px} cy={p2py} r={6} fill={accent} />
      </Svg>
    </View>
  );
}

// ── 3. Derivative Slope: auto-move point along x² ──────────────────────────
export function DerivativeSlopeViz({ accent }: VizProps) {
  const xs = [W * 0.30, W * 0.42, W * 0.55, W * 0.68, W * 0.78];
  const [i, setI] = useState(0);
  useEffect(() => {
  ...
  const dotXVal = xs[i];
  const xRel = (dotXVal - W / 2) / s;
  const dotYVal = cy - xRel * xRel * 3;
  ...
  return (
    <View>
      <Svg width={W} height={H}>
        ...
        <Path d={pts.join(' ')} stroke={colors.text2} strokeWidth={1.5} fill="none" />
        <Circle cx={dotXVal} cy={dotYVal} r={7} fill={accent} />
      </Svg>
    </View>
  );
}

// ── 4. Optimization Box: auto-cycle cut size ───────────────────────────────
export function OptimizationBoxViz({ accent }: VizProps) {
  const cutVals = [W * 0.20, W * 0.35, W * 0.50, W * 0.65, W * 0.80];
  const [i, setI] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setI(p => (p + 1) % cutVals.length), 1500);
    return () => clearInterval(t);
  }, []);
  const cutPos = cutVals[i];
  ...
  return (
    <View>
      <Svg width={W} height={H}>
        ...
        <Circle cx={cutPos} cy={H - 36} r={7} fill={accent} />
...
// ── 5. Riemann Sum: auto-cycle n ───────────────────────────────────────────
export function RiemannSumViz({ accent }: VizProps) {
  const counts = [4, 8, 16, 32];
  const [idx, setIdx] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setIdx(p => (p + 1) % counts.length), 1800);
    return () => clearInterval(t);
  }, []);
  const n = counts[idx];
...
// ── 6. FTC Area: auto-cycle upper bound ────────────────────────────────────
export function FtcAreaViz({ accent }: VizProps) {
  const bVals = [W * 0.35, W * 0.50, W * 0.65, W * 0.80];
  const [i, setI] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setI(p => (p + 1) % bVals.length), 1800);
    return () => clearInterval(t);
  }, []);
  const bX = bVals[i];
  ...
  return (
    <View>
      <Svg width={W} height={H}>
        ...
        <Rect x={lx} y={cy - 50} width={Math.max(0, bX - lx)} height={50} fill={accent + '22'} />
        <Path d={pts.join(' ')} stroke={accent} strokeWidth={2} fill="none" />
        {/* Lower bound */}
        ...
        {/* Upper bound */}
        <Line x1={bX} y1={cy - 55} x2={bX} y2={cy + 5} stroke={accent} strokeWidth={2} />
        <SvgText x={bX} y={cy + 18} fill={colors.text3} fontSize={10} textAnchor="middle">b</SvgText>
      </Svg>
    </View>
  );
}

// ── 7. u-Substitution Chain: auto-peel layers ──────────────────────────────
...
// ── 8. Continuity Test: auto-cycle gap ─────────────────────────────────────
export function ContinuityTestViz({ accent }: VizProps) {
  const gaps = [30, 20, 10, 0];
  const [i, setI] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setI(p => (p + 1) % gaps.length), 1500);
    return () => clearInterval(t);
  }, []);
  const gapY = gaps[i];
  ...
  return (
    <View>
      <Svg width={W} height={H}>
        ...
        {/* Moving piece */}
        <Circle cx={splitX} cy={dotCy} r={6} fill={accent} />
        {/* Hole */}
...
// ── 9. MVT Secant: auto-toggle parallel tangent ────────────────────────────
export function MvtSecantViz({ accent }: VizProps) {
  const [showTangent, setShowTangent] = useState(false);
  useEffect(() => {
    const t = setInterval(() => setShowTangent(p => !p), 1800);
    return () => clearInterval(t);
  }, []);
...
// ── 10. L'Hôpital Zoom: auto-cycle zoom levels ─────────────────────────────
export function LhopitalZoomViz({ accent }: VizProps) {
  ...
  useEffect(() => {
    const t = setInterval(() => setZoom(p => (p + 1) % levels.length), 1800);
    return () => clearInterval(t);
  }, []);
  const z = levels[zoom];
...
// ── 11. Related Rates Cone: auto-cycle water level ─────────────────────────
export function RelatedRatesConeViz({ accent }: VizProps) {
  const levels = [H * 0.30, H * 0.45, H * 0.60, H * 0.75];
  const [i, setI] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setI(p => (p + 1) % levels.length), 1500);
    return () => clearInterval(t);
  }, []);
  const waterY = levels[i];
  ...
  const ratio = (waterY - H * 0.1) / (H * 0.8);
  ...
  return (
    <View>
      <Svg width={W} height={H}>
        ...
        {/* Water level line */}
        <Line x1={W / 2 - halfW} y1={waterY} x2={W / 2 + halfW} y2={waterY} stroke={accent} strokeWidth={2} />
        {/* Labels */}
...
// ── 12. Implicit Curve: auto-cycle point on ellipse ────────────────────────
export function ImplicitCurveViz({ accent }: VizProps) {
  const angles = [0.4, 1.2, 2.5, 3.8];
  const [idx, setIdx] = useState(0);
  useEffect(() => {
    const t = setInterval(() => setIdx(p => (p + 1) % angles.length), 1800);
    return () => clearInterval(t);
  }, []);
  ...
  const safeEy = Math.abs(ey) < 0.001 ? (ey >= 0 ? 0.001 : -0.001) : ey;
  const slope = ey !== 0 ? -(ry * ry * ex) / (rx * rx * safeEy) : Infinity;
  const tanLen = 40;
